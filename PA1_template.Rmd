...
# create histogram
ggplot(dfc, aes(date, steps)) + geom_bar(stat = "identity") +
...
ggplot(dfc, aes(interval, avg_steps)) + geom_line() +
  ylab("steps") +
  ggtitle("Average Steps per Day by Time Interval")
```
...
for (i in i:nrow(df_new)){
  if (is.na(df_new$steps[i]) == TRUE){
    df_new$steps[i] <- select(filter(dfc, interval == df_new$interval[i]), avg_steps)
  }
}
```
...
ggplot(df_new_sum, aes(date, steps)) + geom_bar(stat = "identity") +
  ggtitle("Steps per Day")
```
...
i <- 1
for (i in i:nrow(df_new)){
  if (weekdays(df_new$date[i]) %in% c("Monday", "Tuesday", "Wednesday", "Thursday", "Friday")){
...
ggplot(dfg, aes(interval, steps)) + geom_line() +
  facet_grid(day ~ .)
```
